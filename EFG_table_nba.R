...
df1 = df %>%
  ...
  mutate(total_two_point_made=total_field_goals_made-total_three_point_field_goals_made)%>%
  mutate(EFG = (total_two_point_made + 1.5 * total_three_point_field_goals_made) / (total_field_goals_attempted))
...
df1 %>%
  ...
  tab_header(title = "Exploring the EFG of the Top-10 NBA Scorers") %>%
  tab_footnote(footnote = "Data:hoopR/NBAstats")%>%
